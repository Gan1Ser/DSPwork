...
​	本课程设计要求为汉宁窗函数，查表可知汉宁窗阻带最小衰减为43.9dB；过渡带带宽$Δw=\frac{3.11\pi}{M}$，根据所涉及的带宽，可据此算出滤波器的阶数为2736。根据采用的正弦噪声频率分别为930，950，970，1000Hz，再根据观察原始音频信号的频谱图，相结合系统设计图，设计通带截至频率为$f_p=1110Hz$，阻带截至频率为$f_s=950Hz$，则$f_c=\frac{f_p+f_s}{2}=1030Hz，Δw=f_p-f_s=160Hz$，代入即可秋娥的滤波器阶数。
